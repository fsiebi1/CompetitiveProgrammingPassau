...
// wh wahrscheinlichkeit pro versuch, wh_hierher wahrscheinlikheit zu dieser abfrage zu kommen, weg der wiederholt werden muss
double rek(double wh, double wh_hierher, double weg) {
    double now = wh * wh_hierher * weg;          // wh es jetzt zu schaffen
    if (now * weg < 0.00000001)
        return now;
    return now + rek(wh, wh_hierher * (1 - wh), weg);     // wenn es nicht geschafft wurde
}
...
int main() {
    ...
    for(int i = 1; i <= m; i++) {
        for(int j = 0; j < zeit; j++) {
            ...
            if (j >= fail_wait[i])
                dp[i][j] = min(dp[i][j], dp[i - 1][j - fail_wait[i]] + get_there[i] + fail_wait[i] * prob[i]);      // wait
        }
    }
